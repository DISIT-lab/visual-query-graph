...
// 		CONSTRUCT {
// 			?a ?b <http://dati.senato.it/senatore/22701> .
// 			<http://dati.senato.it/senatore/22701> ?c ?d .}
// 			WHERE {
// 				{ ?a ?b <http://dati.senato.it/senatore/22701>}
// 				UNION
// 				{
...
// 				}
		...
		$parent_id=null;
		$title=$_POST['title'];
		$desc=$_POST['desc'];//TODO insert a control for the length of the description based on the dimension on the database.
		...
		if ($_POST['update']=="true" && $parent_id!=null) { //Update case.
			...
			$query="UPDATE graph SET config='".$json_conf ."', timestamp=".time() .",list_endpoints='".$_POST['EPs'] ."',status='".$_POST['status'] ."',title='".$title ."' ,description='".$desc ."' WHERE readwrite_id='".$parent_id ."' ";
			$conf_id=$parent_id;
			...
			$query="INSERT INTO graph (id,mail, sparql_endpoint,initial_uri,config,parent_id,timestamp,readwrite_id,list_endpoints,status,title,description) VALUES ('".$conf_id_r ."','".$email ."','".$_POST['endpoint'] ."','".$_POST['initial_uri'] ."','".$json_conf."','".$parent_id ."',".time() .",'".$conf_id_rw ."','".$_POST['EPs'] ."','".$_POST['status'] ."','".$title ."','".$desc ."')";
		...
		if($_POST['update']!="true"){
			...
			$link='http://'.$_SERVER['HTTP_HOST'].''.substr($_SERVER['REQUEST_URI'],0,strrpos($_SERVER['REQUEST_URI'],'/')). '/index.php?graph=';
// 			$link='http://'.$_SERVER['HTTP_HOST'].''.substr($_SERVER['REQUEST_URI'],0,strrpos($_SERVER['REQUEST_URI'],'.php')). '?graph=';
			$link_r=$link. $conf_id_r;
			$link_rw=$link. $conf_id_rw;
			if($_POST['saprqlSessionId']){
				$link_rw=$link_rw.'&sparql_session_id='.$_POST['saprqlSessionId'];
			}
			if($_POST['sparql_session_next_query_id']){
				$link_rw=$link_rw.'&sparql_session_next_query_id='.$_POST['sparql_session_next_query_id'];
			}
			
			$result->ovwerwriteUrl=$link_rw;
