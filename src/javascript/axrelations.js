...
function axrelations_graph_open(id,div_position,type,endpointforrequest,multi_ep){
	...
	d3.select("#hooch_sparql_navigation_patRes").transition().duration(500).style("display","none");
	d3.select("#hooch_sparql_navigation_patResMenu").transition().duration(500).style("display","none");
	
	d3.select("#hooch_sparql_navigation_patLink").transition().duration(500).style("display","none");
	d3.select("#hooch_sparql_navigation_istRes").transition().duration(500).style("display","none");
	d3.select("#hooch_sparql_navigation_svgLinkLineRem").transition().duration(500).style("display","none");
	...
	force = d3.layout.force()
		.gravity(0.05) // default was 0.05
		.friction(0.7)// Specifies how fast decrease the speed. For default is set to 0.9
		.charge(-2000) // default was 2000
		.linkDistance(50) // default 50
		.size([w, h]);
	...
	function axrelations_first_call(){
		...
		//axrelations_graph_div is the main div, where all elements will be added
	...
	d3.json("request.php?configuration=g",function(json){
		...
		d3.select("input#axrelations_undo")
		.attr("id","axrelations_undo")
		...
		.on("mouseup",function(){
			...
			// HoochSparql undoOperation
			// new operations are:
			// 1) HoochSparqlAddPatternNode
			// 2) HoochSparqlDropLinkOverRes
			// 3) HoochSparqlDropResOverPL
			// 4) HoochSparqlDropResOverRes
			// 5) HoochSparqlSetOptionalLink
			
			// 6) HoochSparqlCutPatterniLink
			// 7) HoochSparqlCutParentPatterniLink
			// 8) HoochSparqlCutTargetPatterniLink
			// 9) HoochSparqlDeletePatterniResource
			
			if(operation.type=="HoochSparqlAddPatternNode"){
				hoochDeleteSparqlNodeFromView(operation.node);
				restart();
			}
			if(operation.type=="HoochSparqlDropLinkOverRes"){
				hoochRevertDropPLoverRes(operation);
				restart();
			}
			
			if(operation.type=="HoochSparqlDropResOverPL"){
				hoochRevertDropResOverPl(operation);
				restart();
			}
			
			if(operation.type=="HoochSparqlDropResOverRes"){
				hoochRevertDropResOverRes(operation);
				restart();
			}
			if(operation.type=="HoochSparqlSetOptionalLink"){
				hoochRevertLinkOptionalStatus(operation);
				restart();
			}
			if(operation.type == "HoochSparqlCloneRelation"){
				hoochRevertCloneRelation(operation);
				restart();
			}
			if(operation.type == "HoochSparqlQueryZero"){
				revertQueryZero(operation);
				restart();
			}
			if(operation.type == "HoochSparqlSwapPatterniLink"){
				hoochRevertSwapPatterniLink(operation);
				restart();
			}
			
			if(operation.type == "HoochSparqlCutPatterniLink"){
				revertHoochCutPatternLink(operation);
				restart();
			}
			
			
			if(operation.type == "HoochSparqlCutParentPatterniLink"){
				revertHoochCutParentPatternLink(operation);
				restart();
			}
			
			if(operation.type == "HoochSparqlCutTargetPatterniLink"){
				revertHoochCutTargetPatternLink(operation);
				restart();
			}
			
			if(operation.type == "HoochSparqlDeletePatterniResource"){
				revertDeletePatternResource(operation);
				restart();
			}
			
			if(operation.type == "HoochSparqlExpandResultMore"){
				hoochRevertExpandMore(operation);
				restart();
			}
			
			if(operation.type == "HoochSparqlTogglePredicate"){
				hoochRevertToggle(operation);
				restart();
			}
			
			
			if(stack_lifo.length==0){ //de actives the button
				d3.select("input#axrelations_undo").attr("disabled","disabled")
				.attr("src",buttons[2]);
			}
		})
	   ...
	   function axrelations_fullscreen_open(){//Opens the axrelastions_graph full-screen and extend the graph container.
		...
		//Creates the buttons.   
		d3.select("input#axrelations_help").attr("type","image")
		.attr("src",buttons[4])
		.attr("alt","Help")
		.attr("title","Help")
		.on("mouseup",function(){
		...
		d3.select("input#axrelations_screenfull").attr("type","image")
		.attr("src",buttons[9])
		...
		d3.select("input#axrelations_screenreduce").attr("type","image")
		.attr("src",buttons[10])
		...
		d3.select("input#axrelations_recenter").attr("type","image")
		.attr("src",buttons[5])
		.attr("alt","Re-center")
		...
		d3.select("input#axrelations_zoom_in").attr("type","image")
		.attr("src",buttons[6])
		.attr("alt","Zoom In")
		.attr("title","Zoom In");
		if($("body.Android").length>0){
			d3.select("input#axrelations_zoom_in")
		...
		d3.select("input#axrelations_zoom_reset").attr("type","image")
		.attr("src",buttons[7])
		.attr("alt","Zoom Reset")
		.attr("title","Zoom Reset");
		if($("body.Android").length>0){
			d3.select("input#axrelations_zoom_reset")
			.on("touchend",function(){
				var s=zoom.scale();
				var t=zoom.translate();
				var w2=d3.select("#axrelations_graph_svg").attr("width")/2;
		...
		d3.select("input#axrelations_zoom_out").attr("type","image")
		.attr("src",buttons[8])
		.attr("alt","Zoom Out")
		.attr("title","Zoom Out")
		.on("mousedown",function(){
		...
		d3.select("input#axrelations_save_configuration").attr("type","image")
		.attr("src",buttons[13])
	...
	$(document).bind('click', function(e){
		if(e.target && e.target.parentNode){		
			if(e.target.parentNode.className && e.target.parentNode.className.baseVal=="node"){
				...
				$("#hooch_sparql_navigation_patRes").css("top",e.pageY).css("left",e.pageX + 80);
				$("#hooch_sparql_navigation_patResMenu").css("top",e.pageY).css("left",e.pageX);
				$("#hooch_sparql_navigation_patLink").css("top",e.pageY).css("left",e.pageX);
				$("#hooch_sparql_navigation_istRes").css("top",e.pageY).css("left",e.pageX + 80);
...
function restart() {//Function for update the visualization and d3
	...
	.on("mouseover",function(d){
		...
		// the offset of the container must be added to the d3.mouse coordingate
		if(d.source.sparqlHooch == true || d.target.sparqlHooch == true){
			var x = d3.mouse(d3.select("#axrelations_graph_svg").node())[0] ;
			var y = d3.mouse(d3.select("#axrelations_graph_svg").node())[1] ;
			
			var xCorr = $('#axrelations_graph_container').offset().left;
			var yCorr =  $('#axrelations_graph_container').offset().top;
			x = x +  xCorr - 20;
			y = y + yCorr - 20;

			$("#hooch_sparql_navigation_svgLinkLineRem").css("top", y).css("left",x);
			$("#hooch_sparql_navigation_svgLinkLineRem").show();
			
			$("#hooch_sparql_navigation_btn_svgLinkRemoval").unbind('click').click(function(){//Specifies another trigger.
				hoochCutSinglePatternLink(d);
			});
		}
	...
	d3.select("#hooch_sparql_navigation_btn_svgLinkRemoval")
	.on("mouseout",function(d){
		d3.event.preventDefault();
		$("#hooch_sparql_navigation_svgLinkLineRem").hide("fast");
	});
	...
	function focusIn(element,first){
		...
		{			
			for(var i=0;i<links.length;i++){
				if(links[i].source.type==element.type&&links[i].source.id==element.id){
					if(node[0]){
						for(var j=0;j<node[0].length;j++){
							if(node[0][j] && node[0][j].__data__.type==links[i].target.type&&node[0][j].__data__.id==links[i].target.id){
				...
				else if(node[0] && links[i].target.type==element.type&&links[i].target.id==element.id){
					for(var j=0;j<node[0].length;j++){
						if(node[0][j] && node[0][j].__data__.type==links[i].source.type&&node[0][j].__data__.id==links[i].source.id){
							var node_passed=Array();
							node_passed.push(node[0][j]);
							focusIn(node_passed,false);
							break;
	...
	function focusOut(element,first){
		...
		{							
			for(var i=0;i<links.length;i++){
				if(node[0] && links[i].source.type==element.type&&links[i].source.id==element.id){
					for(var j=0;j<node[0].length;j++){
						if(node[0][j] && node[0][j].__data__.type==links[i].target.type&&node[0][j].__data__.id==links[i].target.id){
							var node_passed=Array();
							node_passed.push(node[0][j]);
...
//			if (d3.event.defaultPrevented) return; //TODO put me back
			// d3.event.sourceEvent.stopPropagation();
			...
			for(var i=0;i<children.length;i++){
				if(children[i].localName=="image"){
					if( children[i].getAttribute('class')=='node_remove'){
						children[i].setAttribute("display","initial");
					}
				}
			}
...
//			d3.event.sourceEvent.stopPropagation();
	...
	node.append("title").text(function(d){
		...
		if(d.isRelation==true){
			return d.uri+" 'of "+d.father+"'";
		}
		else if(d.type=='more' || d.type=='more_LD' || d.type == 'moreResultSet'){
			return d.name;
		}
		else if(d.name!=d.id) { 
			return d.name+" \n "+d.id;
	...
	.attr("class", function(d){
		if(d.isRelation==true){
			if(d.sparqlHooch == true || d.sparqlHoochResult == true){
				return "queryDontTouch";
			}else if(d.explored==true){
				return "expanded";
			}else{
				return "reduced";
			}
...
function step1(x,y,EP) {//First step for the visualization. It creates a node root and search the relations. (x,y) is the position where place the new root. EP is the EndPoint of this search. 
	...
	if(active_multiple_endpoints===null){
		//--------------Waits until the end of the loading of the configuration.
...
*/
	...
	var parentNode = relation_to_explore.parentNode;
	if(parentNode){
		...
		if(parentPresent == false){
			...
			links.push({source: parentNode, target : relation_to_explore, present : true});
		}else{
		}
	}
	...
	for(var i=0;i<relation_to_explore.elements.length;i++){
		...
		for(var j=0;j<nodes.length;j++){
			if(!nodes[j].isRelation ){
				if(nodes[j].id==relation_to_explore.pid){
					var existent_parent_link=links.filter(function(d){
						if(d.source.id==nodes[j].id && d.target.id==relation_to_explore.id)
						{ 
							return d;
						}
					});
					if(existent_parent_link.length==0 && axrelations_search_node_duplication({source: nodes[j] , target: relation_to_explore, present:true})){
						links.push({source: nodes[j] , target: relation_to_explore, present:true});
					}
					break;
				}
			}
		...
		for(var j=0;j<nodes.length;j++){
			if(!nodes[j].isRelation ){
				...
				if(nodes[j].id==element.id) { 
					if(nodes[j].index && element.index ) {
						if(nodes[j].index == element.index){
							...
							var existent_link=links.filter(function(d){
								if(d.target.id==nodes[j].id && d.source.id==relation_to_explore.id)
								{ 
									return d;
								}
							});
							if(existent_link.length==0 && axrelations_search_node_duplication({source: relation_to_explore, target: nodes[j], present:true})){
								links.push({source: relation_to_explore, target: nodes[j], present:true});
							}
							break;
						}// This is for the cases of add new elements to a node already explored(Case MultiEP).
						else{
						}
					}
					if(nodes[j].EP.indexOf(EPindex)==-1){
						nodes[j].EP.push(EPindex);//If the node is reached by another with a different EndPoint, adds the EndPoint index in the list of the node.
					}
					var existent_link=links.filter(function(d){
						if(d.target.id==nodes[j].id && d.source.id==relation_to_explore.id)
						{ 
					...
					if(relation_to_explore.sparqlHoochResult == true && existent_link.length==0){
						links.push({source: relation_to_explore, target: nodes[j], present:true});
					}else if(existent_link.length==0 && axrelations_search_node_duplication({source: relation_to_explore, target: nodes[j], present:true})){
						links.push({source: relation_to_explore, target: nodes[j], present:true});
					}else{
						rejects_count++;
					}
	...
	if(rejects_count==relation_to_explore.elements.length){
		...
		axrelations_graph_finder(); // here call the deletion
	}
}
...
function open_navigation_panel(e,current) {//Opens the navigation panel with the functions of the tool.
	...
	if(node_to_explore.type=="more" || node_to_explore.type=="more_LD" || node_to_explore.type=="moreResultSet"){//Opens the navigation panel for the more button.
		d3.select("#hooch_sparql_navigation_patRes").transition().duration(500).style("display","none");
		d3.select("#hooch_sparql_navigation_patResMenu").transition().duration(500).style("display","none");
		d3.select("#hooch_sparql_navigation_patLink").transition().duration(500).style("display","none");
		d3.select("#hooch_sparql_navigation_istRes").transition().duration(500).style("display","none");
		...
		if(node_to_explore.sparqlHoochResult == true){
			d3.select("#axrelations_graph_navigation_more_search").transition()
				.duration(500)
				.style("display","none");
		}else{
			d3.select("#axrelations_graph_navigation_more_search").transition()
		...
		$("#axrelations_graph_navigation_more_explore").unbind('click').click(function(){//Specifies another trigger.
			if ( node_to_explore.sparqlHoochResult == true || node_to_explore.type == "moreResultSet"){
				hoochExpandMoreResult(node_to_explore);
			}else{
	...
	if( isQueryModeEnabled() == true && node_to_explore.isRelation == true){
		// turn off the functions for the patres
		if(node_to_explore.sparqlHooch != true){
			d3.select("#hooch_sparql_navigation_patRes").transition()
			  .duration(500)
			  .style("display","inline");
			d3.select("#hooch_sparql_navigation_patResMenu").transition().duration(500).style("display","none");
			d3.select("#hooch_sparql_navigation_patLink").transition().duration(500).style("display","none");
			...
			d3.select("#hooch_sparql_navigation_patLink").transition()
			  .duration(500)
			  .style("display","inline");
			d3.select("#hooch_sparql_navigation_patRes").transition()
			  .duration(500)
	...
	}else if(isQueryModeEnabled() == true && node_to_explore.isRelation != true){
		
		if(node_to_explore.sparqlHooch != true){
			d3.select("#hooch_sparql_navigation_patLink").transition().duration(500).style("display","none");
			d3.select("#hooch_sparql_navigation_istRes").transition().duration(500).style("display","inline");
			d3.select("#hooch_sparql_navigation_svgLinkLineRem").transition().duration(500).style("display","none");
			d3.select("#hooch_sparql_navigation_patRes").transition().duration(500).style("display","none");
			d3.select("#hooch_sparql_navigation_patResMenu").transition().duration(500).style("display","none");
			d3.select("#axrelations_graph_navigation_panel").transition()
			...
			d3.select("#hooch_sparql_navigation_patResMenu").transition().duration(500).style("display","inline");
			d3.select("#hooch_sparql_navigation_istRes").transition().duration(500).style("display","none");
			d3.select("#hooch_sparql_navigation_svgLinkLineRem").transition().duration(500).style("display","none");
			d3.select("#hooch_sparql_navigation_patRes").transition().duration(500).style("display","none");
			d3.select("#hooch_sparql_navigation_patLink").transition().duration(500).style("display","none");
	...
	if(node_to_explore.isRelation==true ){ //For a relations doesn't show the menu, explore immediately.
		...
		// Hooch add current != null to prevent wild error
		
		
		// TEST && ! node_to_explore.sparqlHoochResult ==true
		if(! node_to_explore.sparqlHooch==true  && current != null){
			//TOCHECK is necessary with the explorations with multiple step
			...
			d3.select(children_circle[0]).transition()
			  .attr("class",function(d){
				  if(d.explored==true){
					  if(d.inbound==true){
						  return "inbound_reduced focus";
					  }else{
						  return "reduced focus";
					  }
				  }else{
					  if(d.inbound==true){
						  return "inbound_expanded focus";
					  }
					  return "expanded focus"
				  };
	...
	$("#axrelations_graph_navigation_panel_info").unbind('click').click(//specifico un altro trigger
		function(){
			...
			var id2explore = null;
			if(node_to_explore.sparqlHoochResult == true){
				id2explore = node_to_explore.idForExplore;
			}else{
				id2explore = node_to_explore.id; 
			}
			
			
			d3.json("request.php?literals="+encodeURIComponent(id2explore)+"&sparql="+EP,function(json){	// Downloads the list of relations.
	...
	$("#axrelations_graph_navigation_panel_search").unbind('click').click(function(){//Specifies another trigger.
		...
		if(!EP)EP=endpoint;
		d3.json("request.php?query_search="+encodeURIComponent(node_to_explore.id)+"&sparql="+EP,function(json){// Downloads the list of query.
...
//	 TODO handle me
	...
	$("#hooch_sparql_navigation_patRes_clone").unbind('click').click(function(){//specifico un altro trigger	 
		hoochCloneIstanceLink(node_to_explore);
	});
	$("#hooch_sparql_navigation_patRes_filter").unbind('click').click(function(){//specifico un altro trigger	 
		hoochFilterPatternLink(node_to_explore);
	});
	$("#hooch_sparql_navigation_patRes_optional").unbind('click').click(function(){//specifico un altro trigger	 
		hoochOptionalPatternLink(node_to_explore);
	});
	$("#hooch_sparql_navigation_patRes_toggleInbound").unbind('click').click(function(){//specifico un altro trigger	 
		hoochToggleInbounPatternLink(node_to_explore);
	});
	
	
	
	$("#hooch_sparql_navigation_patLink_swap").unbind('click').click(function(){//specifico un altro trigger	 
		hoochSwapPatternLink(node_to_explore,true);
	});
	$("#hooch_sparql_navigation_patLink_cut").unbind('click').click(function(){//specifico un altro trigger	 
		hoochCutPatternLink(node_to_explore);
	});
	
	$("#hooch_sparql_navigation_patLink_cut_parent").unbind('click').click(function(){//specifico un altro trigger	 
		hoochCutParentFromPatternLink(node_to_explore);
	});
	
	$("#hooch_sparql_navigation_patLink_cut_target").unbind('click').click(function(){//specifico un altro trigger	 
		hoochCutTargetFromPatternLink(node_to_explore);
	});
	
	
	
	$("#hooch_sparql_navigation_istRes_qyeryZero").unbind('click').click(function(){//specifico un altro trigger	 
		queryZero(node_to_explore);
	});
	...
	if(node_to_explore.type=="uri"||node_to_explore.type=="LD" ){
		$("#axrelations_graph_navigation_panel_open").attr("disabled",null);//Shows the open button
	}else{
		$("#axrelations_graph_navigation_panel_open").attr("disabled","disabled");
	}
...
function explore(node_to_explore,save_status,save_type,add_info,details){
	...
	if(node_to_explore.sparqlHoochResult){
	}
	if(details){ 
		...
		// TODO here status for expand reduce
		
		if(!node_to_explore.relations){
			node_to_explore.explored=false;
		}
		
		if((graph_status=='to_explore' || graph_status=='none')&& node_to_explore.explored==false && details=='more'){ 
			delete node_to_explore.relations;
			node_to_explore.explored=false;
			...
			delete node_to_explore.explored;
			if(save_status){
		...
		if(node_to_explore.explored==false){ //If the node wasn't already been explored.
			...
			node_to_explore.explored=true;			
		} else{ //Deletes the subgraph with an algorithm of deep search over the link. //Prunes the branch.
			if(save_status==true){	//Save the operation on stack
				...
				for(var i=0;i<nodes.length;i++){
					operation.nodes[i]=nodes[i];
				}
				operation.links=Array();
				for(var i=0;i<links.length;i++){
					operation.links[i]=links[i];
				}
...
function hoochNodeLabeling(nodeId){
	var stack_of_nodes=Array();
	var salvation = false;
	for(var i=0;i<nodes.length;i++){
		if(nodes[i].id==nodeId){
			nodes[i].reachedNode=true;//Starts the labelling from the root. For more roots more pushes!
			stack_of_nodes.push(nodes[i]);		
		}
		else{ 
			nodes[i].reachedNode=false;
		}
	}
	while(stack_of_nodes.length!=0){
		var node_reached=stack_of_nodes.pop();			
		for(var i=0;i<links.length;i++){
			if(links[i].source.index==node_reached.index && links[i].target.reached==false){
				if(links[i].target.root == true){
					salvation = true;
					break;
				}else{
					links[i].target.reachedNode=true;
					stack_of_nodes.push(links[i].target);
				}
			}
		}
...
function element_elimination(source){//Defines the functions for deletion. This function deletes all the edges that have for source 'source'.
	...
	for(var i=0;i<links.length;i++){
		...
		if(source.sparqlHoochResult == true && source.isRelation == true 
//				&& source.inbound != true
		) {
			if(source.inbound == true){
				if( links[i].source.index == source.index){
					var goWithCut = false;
					
					
					var trgId = links[i].target.id;
					for(x in source.elements){
						if ( source.elements[x].id == trgId){
							goWithCut = true;
							break;
						}
					}
				...
				if(links[i].source.index == source.index){
					...
					if(source.parentNode && ( source.parentNode.root==true || source.parentNode.reached == true)){
						goWithCut = true;
					}else{
						var trgId = links[i].target.id;
						var trgReach = links[i].target.reached;
						if(trgReach == false){
							// try to follow a root
							if(hoochNodeLabeling(links[i].target.id) == false){
								goWithCut = true;
							}
						}
					}
				}else if(links[i].target.index == source.index  ){
					if((source.parentNode && source.parentNode.root == true)){
					}else if(source.parentNode && source.parentNode.reached == true){
						// if the parent of the predicate is reached by the root, then pid must be saved
					}else{
						var trgId = links[i].source.id;
						if(trgId == source.pid){
							for(x in source.elements){
								var elem4check = source.elements[x];
								if ( elem4check.reached != true ){
									// parent relation must not be cut
									goWithCut = false;
									break;
								}else{
									goWithCut = true;
		...
		} else if( links[i].source.index == source.index ){//Search the edges where 'source' is the source or trgethe target.
			
			//HOOCH added to not remove sparql elements
//			if(links[i].source.sparqlHooch == true || links[i].target.sparqlHooch == true){
			if(source.sparqlHooch == true ){
				// HOOCH, if one of the nodes linked is a sparql element, the edge will not be removed
			}else{
...
function element_elimination_allway(source){
	...
	// TEST || source.sparqlHoochResult == true
	if( ( source.sparqlHooch == true ) && source.isRelation != true){
		// need custom undo
		deleteResourceFromGraph(source);
		
	}else if(( source.sparqlHooch == true ) && source.isRelation == true){
		// need custom undo
		hoochCutPatternLink(source);
		
	}else if(source.sparqlHooch != true && source.isRelation == true){
		axrelations_save_status("node_remove",source);//Saves like a focus.
...
function axrelations_graph_finder(){//Function that completes the elimination. Removes all the isolated nodes with a breadth search.
	// deletes every node wich is not reached from a root
	var stack_of_nodes=Array();
	for(var i=0;i<nodes.length;i++){
		if(! nodes[i]){
			continue;
...
//	request.php?uri=http%3A%2F%2Fdati.senato.it%2Fsenatore%2F22759&sparql=http://dati.senato.it/sparql
	
//	|| node_to_explore.sparqlHoochResult
	var exploreSparqlResult = true;
//	if(node_to_explore.sparqlHoochResult){
//		for(x in node_to_explore.realtions){
//			exploreSparqlResult = false;
//			break;
//		}
//	}
	
	//
	if( node_to_explore.relations!=null ||node_to_explore.relations==null  || node_to_explore.single_search==true || node_to_explore.sparqlHoochResult) { // If the relations aren't already been loaded.
		...
		if(node_to_explore.type!="more" && node_to_explore.type!="more_LD") { //Normal search. Gets the properties of a node from the server.
			...
			var nodeId2explore = node_to_explore.id;
			if(node_to_explore.sparqlHoochResult == true){
				nodeId2explore = node_to_explore.idForExplore;
			}
			if(active_multiple_endpoints){
				EP_to_add=axr_EP_for_node(nodeId2explore);
				//Checks if the elements in EP_to_add are already present in endpoints_list and in node_to_explore.EP, else adds that to each one.
				var list_index;
				if(EP_to_add){
					list_index=axr_add_EPs(EP_to_add);
			...
			for(var p=0;p<EndPoints.length;p++){//Makes a search for all the EP of the node to explore.
				...
				//Prepares the url for the request.
				if(!EP){
					EP = node_to_explore.EP;
				}
				var request_url="request.php?";
				if(node_to_explore.sparqlHoochResult || node_to_explore.type=='patternResource'){
					request_url = request_url + "uri="+encodeURIComponent(nodeId2explore)+"&sparql="+node_to_explore.EP;
				}else{
					request_url = request_url +node_to_explore.type+"="+encodeURIComponent(nodeId2explore)+"&sparql="+EP;
				}
				if(active_multiple_endpoints){
					if( EndPoints[p]!=-1 && node_to_explore.type=="LD" )request_url="request.php?uri="+encodeURIComponent(nodeId2explore)+"&sparql="+EP;
				}
...
function cloneNode4save(node){
	
	var clone = new Object;
	
	clone.inbound = node.inbound;
	clone.uri = node.uri;
	clone.EP = node.EP;
	clone.explored = node.explored;
	clone.fixed = node.fixed;
	clone.id = node.id;
	clone.img = node.img;
	clone.index = node.index;
	clone.isRelation = node.isRelation;
	clone.name = node.name;
	clone.pending = node.pending;
	clone.px = node.px;
	clone.py = node.py;
	clone.type = node.type;
	clone.weight = node.weight;
	clone.x = node.x;
	clone.y = node.y;
	clone.father = node.father;
	
	if(clone.type=='moreResultSet'){
		clone.resultId = node.resultId;
		clone.count = node.count;
		if(node.sourcePredicateRefId){
			clone.sourcePredicateRefId = node.sourcePredicateRefId; 
		}else if(node.sourcePredicateRef){
			clone.sourcePredicateRefId = node.sourcePredicateRef.id;
		}
		clone.targetNodesId = Array();
		for(idx in node.targetNodes){
			clone.targetNodesId.push(node.targetNodes[idx].id);
		}
	}
	
	if(clone.type=='patternResource'){
		clone.type='uri';
	}else if(clone.type=='patternLink'){
		clone.type=clone.name;
	}
	
	clone.root = node.root;
	
	if(node.relations){
		if(node.relations.length > 0){
			clone.relations = Array();
			for(var i=0;i<node.relations.length;i++){
				clone.relations[i] = cloneNode4save(node.relations[i]);
			}
		}
	}
	...
	if(node.elements){
		if(node.elements.length > 0){
			...
			for(var i=0;i<node.elements.length;i++){
				clone.elements[i] = cloneNode4save(node.elements[i]);
			}
		}
	}
...
function save_current_configuration(email,update){
	...
	*/
	...
	for(var i=0;i<nodes.length;i++){
//		&& nodes[i].sparqlHoochResult != true
		if(nodes[i].sparqlHooch != true){
//			configuration.nodes[i] =cloneNode4save(nodes[i]);
			configuration.nodes.push(cloneNode4save(nodes[i]));
		}
	}
	...
	if(zoom.scale())zoom_scale = zoom.scale();
	if(zoom.translate())zoom_translate = zoom.translate();
	configuration.zoom_scale=zoom_scale;
	configuration.zoom_translate=zoom_translate;
	...
	for(var i=0;i<links.length;i++){
		// hooch avoid to save query
		if(links[i].source.sparqlHooch != true && links[i].target.sparqlHooch != true
//		   && links[i].source.sparqlHoochResult != true && links[i].target.sparqlHoochResult != true
		){
	...
	var save=JSON.stringify(configuration,function(key,value){
		// avoid cyclic reference on my nodes!
		if(this.fixed!=true &&(key=="x"||key=="y"||key=="py"||key=="px") ){ 
			return undefined;
		}
		return value;
	});
	...
	// hooch sparqlid
	$.post("save_configuration.php",{'sparql_session_next_query_id':sparqlCurrentQueryId,'saprqlSessionId':sparqlSessionId,'email':email,'update':update,'endpoint':endpoint,'initial_uri':initial_uri,'parent_id':parent_id,'save':save,'EPs':EPs,'status':status_opt,'title':JSON.stringify(current_graph_title),'desc':JSON.stringify(current_graph_desc)},function(data){
...
function retrieve_configuration(code) {// Retrieves the configuration identified by 'code'. load_config
	if(code!=null){
		d3.json("request.php?retrieve_configuration="+code,function(json) {
			...
			//retrieves the zoom parameters.
			if(json.zoom_scale && json.zoom_translate && zoom){
				zoom.translate(json.zoom_translate);
				zoom.scale(json.zoom_scale);
				vis.attr("transform", "translate("+zoom.translate()+")"+" scale("+zoom.scale()+")");
			}
			restart();
			hoochSetRelationOnRetrieved();
...
function clear_graph(){
	...
	d3.select("input#axrelations_undo").attr("disabled","disabled")//de actives the button
	.attr("src",buttons[2]);
...
function hoochSetRelationOnRetrieved(){
	// set relations on all node based on the link status
	for(var i in nodes){
		var rels = nodes[i].relations;
		if(! rels) {
			for(y in links){
				var cLink = links[y];
				if(!nodes[i].isRelation && cLink && cLink.source == nodes[i]) {
					if(!nodes[i].relations) {
						nodes[i].relations = [];
					}
					if(cLink.target) {
						nodes[i].relations.push(cLink.target);
						cLink.target.parentNode = nodes[i];
					}
				}
			}	
		}
...
function axrelations_share(current_node) {
	...
	// var multi_ep_opt=$("#log_options input#multiple_ep");
	var multi_ep_opt=$(".endpoint.active input#multiple_ep");
