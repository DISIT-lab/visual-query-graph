...
// Hooch added
function hoochGetAllPredicates($hoochSparqlEndpoint,$curQuery,$json){
	require_once( "sparqllib.php" );
	require_once( "config.php" );
	
	hoochLog("Hooch Connecting to ".$hoochSparqlEndpoint );
	
	hoochLog("Hooch Query is ".$curQuery );

	hoochLog("Hooch JSON is ".$json );
	
	$jsonIN = json_decode($json,true);
	foreach ($jsonIN as $key => $jsons) { // This will search in the 2 jsons
		hoochLog("JSON KEY is " . $key);
		foreach($jsons as $key => $value) {
			hoochLog("JSON VALUE is " . $key . " -> " . $value);
		}
	}
	$finalQ = "SELECT DISTINCT  * WHERE { " . $curQuery . " }";
	hoochLog("Query to send is " .$finalQ );
	$db = sparql_connect( $hoochSparqlEndpoint );
	if( $db->alive() ){
		sparql_ns( "rdfs","http://www.w3.org/2000/01/rdf-schema#" );
		sparql_ns("rdf","http://www.w3.org/1999/02/22-rdf-syntax-ns#");
		sparql_ns( "foaf","http://xmlns.com/foaf/0.1/" );
		sparql_ns("osr", "http://dati.senato.it/osr/" );
		$q = $finalQ;
		$result = sparql_query( $q );
		if($result){
			hoochLog("Executed query");
			hoochLog("Number of rows: ".$result->num_rows( $result )." results.");
			return $result;
		}else{
			$errorMsg = sparql_errno() . ": " . sparql_error();
			return $errorMsg;
		}
		
	}else{
		mysparql_error($hoochSparqlEndpoint." NOT ALIVE\n".$db->error."\n-----------");
		return "NOT alive";
	}
}

function hoochEnumerateLiterals($hoochSparqlEndpoint,$where,$resName){
	
	require_once( "sparqllib.php" );
	require_once( "config.php" );
	
	hoochLog("Literals for " . $resName);
	
	$jsonIN = json_decode($where,true);
	
	$whereClause = buildWhereClause($jsonIN,false);
	$literalQuery = "SELECT DISTINCT ?p "
					." WHERE { ?" . $resName ." ?p ?o . "
					. $whereClause							
					." FILTER(isLiteral(?o)) } ORDER BY ?p";
	
	hoochLog("Final literals is " . $literalQuery);
	...
	if( $db->alive() ){
		...
		if($result){
			...
			$ret->res = null;
			$errorMsg = sparql_errno() . ": " . sparql_error();
			$ret->errMsg = "No Result : " . $errorMsg;
		}
	}else{
		mysparql_error($hoochSparqlEndpoint." NOT ALIVE\n".$db->error."\n-----------");
		$ret->res = null;
		$ret->errMsg = "NOT alive";
	}
	return $ret;
...
function hoochGetSessionId(){
	...
	if(!mysqli_autocommit($dbhandle,FALSE)){
		mysqli_close($dbhandle);
		$ret->err="Unable to start MySQL transaction";
	}else{
		$ret->session= hoochCheckSession($dbhandle, -1);
	}
	return JSON_ENCODE($ret);
}

function hoochCheckSession($dbhandle, $sessionId){
	$query = "SELECT 1 FROM queries WHERE sessionid = " . $sessionId; //. $sessionId;
	// 	hoochLog("Query " . $query);
	$result = mysqli_query($dbhandle,$query);
	if(!$result){
		hoochLog(mysqli_error($dbhandle));
		// failure
		$sessionId = -1;
	}else{
		if(mysqli_num_rows($result) > 0){
			// session exists nothing to do
				// 			hoochLog("Session found");
		}else{
			// session does not exists, create a new one
			// 			hoochLog("Session not found");
			$query = "SELECT max(sessionid) FROM queries";
			$result = mysqli_query($dbhandle,$query);
			if (!$result){
				// failure
				hoochLog(mysqli_error($dbhandle));
				...
				if(mysqli_num_rows($result) > 0){
// 					hoochLog("Session id found");
					$row = mysqli_fetch_row($result);
					$sessionId = $row[0]+1;
// 					hoochLog("Session id is " + $sessionId);
...
function hoochNextSession($dbhandle,$sessionId){
	$query = "SELECT 1 FROM queries WHERE sessionid = " . $sessionId; //. $sessionId;
// 	hoochLog("Query " . $query);
	$result = mysqli_query($dbhandle,$query);
	if(!$result){
...
// 			hoochLog("Session found");
			...
			// session does not exists, create a new one
// 			hoochLog("Session not found");
			$query = "SELECT max(sessionid)+1 FROM queries";
			$result = mysqli_query($dbhandle,$query);
			if (!$result){
				// failure
				hoochLog(mysqli_error($dbhandle));
...
// 					hoochLog("Session id not found");
// 					hoochLog("insert " . $insert);

					// dead code, never goes here
					
					"INSERT INTO queries ".
					"(sessionid, queryid,jsonQuery,launchTime,Status,sparqlQuery,sparqlEndpoint,jsonResult,endTime)" .
					" VALUES ( " .
					$sessionId . " , " .
					" 0 , " .
					"'' , " .
...
function hoochNextQueryId($dbhandle,$sessionId){
	$nextId = -1;
	if($sessionId < 0){
		// failure occured
		$nextId = -1;
	}else{
		$query = "SELECT max(queryId)+1 as nextQueryId FROM queries WHERE sessionId = " . $sessionId;
		$result = mysqli_query($dbhandle,$query);
		if (!result){
			$nextId = -1;
		}else{
...
function hoochSaveQuery($hoochSparqlEndpoint,$json,$sessionId,$sparqlQuery){
	...
	if(!mysqli_autocommit($dbhandle,FALSE)){
		mysqli_close($dbhandle);
		return "Unable to start MySQL transaction";
	}
	
	$sessionId = hoochNextSession($dbhandle, $sessionId);
	if($sessionId < 0){
	...
	$queryId = hoochNextQueryId($dbhandle, $sessionId);
	if($queryId < 0){
		mysqli_rollback($dbhandle);
		mysqli_close($dbhandle);
		return "Invalid query session";
	}
	
	hoochLog("Session " . $sessionId . " Query " . $queryId);
	
	$escapedJson = mysqli_real_escape_string($dbhandle,$json);
	$escapedSparql = mysqli_real_escape_string($dbhandle,$sparqlQuery);
	$escapedEP = mysqli_real_escape_string($dbhandle,$hoochSparqlEndpoint);
	
	$updateQuery = 	"INSERT INTO queries ".
						"(sessionid, queryid,jsonQuery,launchTime,Status,sparqlQuery,sparqlEndpoint,jsonResult,endTime)" .
					" VALUES ( " .
						$sessionId . " , " .
						$queryId . " , " .
	...
	hoochLog("Sessione insert " . $updateQuery);
	...
	return array($sessionId,$queryId);
}

function hoochUpdateQueryResult($sessionId,$queryId,$result){
	...
	if (mysqli_connect_errno()) {
		hoochLog("Failed to connect to MySQL: " . mysqli_connect_error());
	}
	if(!mysqli_autocommit($dbhandle,FALSE)){
		mysqli_close($dbhandle);
		hoochLog("Unable to start MySQL transaction");
	}
	$escapedRes = "";
	$status = 'Executed - OK';
	if($result == null){
		$status = 'Executed - No result';
	}else{
		$escapedRes = mysqli_real_escape_string($dbhandle,$result);
	}
	
	$update = 	"UPDATE queries " .
				" SET " .
				" Status = '" . $status . "', " .
				" jsonResult = '" . $escapedRes . "', " .
				" endTime = now() " .
				" WHERE sessionid = " . $sessionId . " AND queryid = " . $queryId;

	if (!mysqli_query($dbhandle,$update)){
		$error = mysqli_error($dbhandle);
		mysqli_rollback($dbhandle);
		mysqli_close($dbhandle);
		hoochLog("Update query result error : " . $error);
	}else{
		mysqli_commit($dbhandle);
		mysqli_close($dbhandle);
	}
...
function hoochLoadQueryPanel($ep,$sessionId){
	
	$sessionQueries = new stdClass();
	...
	if (mysqli_connect_errno()) {
		hoochLog("Failed to connect to MySQL: " . mysqli_connect_error());
		$sessionQueries->error = "Can't connect to DB, ask the system admin to fix it.";
		return JSON_ENCODE($sessionQueries);
	}
	
	
	
	$escapedEp = mysqli_real_escape_string($dbhandle,$ep);
	
	
	if(is_numeric($sessionId) == false){
		$sessionId = -1;
	}
	
	$query = "select * from queries where sessionid = " . $sessionId . " and sparqlEndpoint='" . $escapedEp . "'";
	
	
	hoochLog("EP " . $ep . " escaped " . $escapedEp . " mysql " . $query );
	
	$result = mysqli_query($dbhandle,$query);
	if(!$result){
		$error = mysqli_error($dbhandle);
		hoochLog("Loading query panel error : " . $error);
		$sessionQueries->error = "Error in SQL query, ask the developers to fix it.";
	}else{
		$numRows = mysqli_num_rows($result);
		hoochLog("Loading query panel ok. Num rows : " . $numRows);
		$queries = array();
		while($row =  mysqli_fetch_object($result)){
			array_push($queries,$row);
		}
		$sessionQueries->numQuery = $numRows;
		$sessionQueries->queries = $queries;
		mysqli_free_result($result);
	}
	mysqli_close($dbhandle);
	return JSON_ENCODE($sessionQueries);
...
// funzione finale per esecuzioen delle query
function hoochQueryJSON($hoochSparqlEndpoint,$json,$clientSession){
	
	if(!is_numeric($clientSession)){
		$clientSession = 0;
	}
	
// 	ini_set('memory_limit', '-1');
	
	error_log("Inside test json"."\n", 3, "/var/log/apache2/hooch.log");
	
	require_once( "sparqllib.php" );
	require_once( "config.php" );
	
	hoochLog("Building query" );
	
	$jsonIN = json_decode($json,true);
	
	$whereClause = " WHERE { ";
	
	$whereClause = $whereClause . buildWhereClause($jsonIN,true);
	
	$whereClause = $whereClause . " } ";
	
	$finalQ = "SELECT DISTINCT  * " . $whereClause;
	...
	if( $db->alive() ){
		...
		$sessionData = hoochSaveQuery($hoochSparqlEndpoint, $json, $clientSession, $finalQ);
				
		if(!is_array($sessionData)){
			$ret->res = null;
			$ret->errMsg = $sessionData;
			return JSON_ENCODE($ret);
		}
		
		$sessionId = $sessionData[0];
		$queryId = $sessionData[1];
		...
		try{
			...
			$errorMsg =  "Errore ignoto"; //sparql_errno() . ": " . sparql_error();
			$ret->res = null;
			$ret->errMsg = $errorMsg;
		}
		
// 		http://localhost/log-edit/index.php?graph=aa4865061004ae40a33923fff82cd4fa&sparql_session_id=67&sparql_session_next_query_id=2
// 		http://localhost/log-edit/index.php?graph=cb221e06dc6ab869249a7660986f5ec5&sparql_session_id=66&sparql_session_next_query_id=2
		...
		if( !$result ) { 
			...
			if($result != null){
				$ret->errMsg = null;
				$ret->res = parseResultoForLog($result,$jsonIN);
			}else{
				$ret->res = null;
				$ret->errMsg = "No Result";
			}	
		}
		$ret->queryId = $queryId;
		$ret->sessionId = $sessionId;
		$ret->resultId = $sessionId ."-" . $queryId;
		$ret = JSON_ENCODE($ret);
		hoochUpdateQueryResult($sessionId, $queryId, $ret);
	}else{
		mysparql_error($hoochSparqlEndpoint." NOT ALIVE\n".$db->error."\n-----------");
		$ret->res = null;
		$ret->errMsg = "NOT alive";
...
function buildWhereClause($jsonIN,$useFilters){
	$whereClause = "";
	foreach ($jsonIN as $key => $queryElem) { // This will search in the 2 jsons
		
		if(!isset($queryElem['value'])){
			continue;
		}
		
		$type = $queryElem['type'];
		$subjValue = $queryElem['value'];
		$subjConst = $queryElem['isConst'];
		if( $type == 'resource'){
			// taking only resource
			// and iterating over the predicates
			$predicates = $queryElem['predicates'];
			foreach($predicates as $pid => $currentPred){
				$predValue = $currentPred['value'];
				$predOpts =  $currentPred['isOptional'];
				$predInbound = $currentPred['inbound'];
				$objArray = $currentPred['objIds'];
				foreach($objArray as $objCounter => $objId){
					$objValue = $jsonIN[$objId]['value'];
					$objConst = $jsonIN[$objId]['isConst'];
						
					// building the right triple based on inbound properties
					if($predInbound == true){
						if( $predOpts == true){
							$currentTriple = "OPTIONAL { " . $objValue . " " . $predValue . "  " . $subjValue  . " . }. ";
						}else{
							$currentTriple = $objValue . " " . $predValue . "  " . $subjValue . ". ";
						}
					}else{
						if( $predOpts == true){
							$currentTriple = "OPTIONAL { " . $subjValue . " " . $predValue . "  " . $objValue . " . }. ";
						}else{
							$currentTriple = $subjValue . " " . $predValue . "  " . $objValue . ". ";
						}
					}
						
						
						
					// if the object of the triple is a variable
					if($objConst == false){
						// then all non resource elment must be filtered out
						if($useFilters == true){
							$objFilters = $jsonIN[$objId]['filters'];
		
							foreach($objFilters as $filterId => $currentFilt){
								hoochLog("Found a filter");
								$fOp = $currentFilt['op'];
								$fVal = $currentFilt['value'];
								$fComp = $currentFilt['comparison'];
								$fPrd = "<" . $currentFilt['predicate'] . ">";
								hoochLog("Filter value is " . $fOp . " - ". $fVal . " - ". $fPrd . " - ");
								if($fOp == 'regex'){
									// is regex
									if( $predOpts == true){
										$currentTriple = $currentTriple . " OPTIONAL { ";
									}
									$currentTriple = $currentTriple . $objValue  . $fPrd . $objValue . "_fnum_" . $filterId . " . ";
									if( $predOpts == true){
										$currentTriple = $currentTriple . " } . OPTIONAL { ";
									}
									$currentTriple = $currentTriple . " FILTER( regex(" . $objValue . "_fnum_" . $filterId . ", \"". $fVal . "\", \"i\" ) ) . ";
									if( $predOpts == true){
									...
									$currentTriple = $currentTriple . " FILTER( " . $objValue . "_fnum_" . $filterId ." ". $fComp ." ". $fVal . " ) . ";
									if( $predOpts == true){
										$currentTriple = $currentTriple . " } . ";
									}
								}elseif($fOp == 'datetime'){
									// if is optional add the optional
									if( $predOpts == true){
										$currentTriple = $currentTriple . " OPTIONAL { ";
									}
									// add the filter named vaiable as triple
									$currentTriple = $currentTriple . $objValue  . $fPrd . $objValue . "_fnum_" . $filterId . " . ";
									if( $predOpts == true){
										$currentTriple = $currentTriple . " } . OPTIONAL { ";
									}
									// apply the filter
									$currentTriple = $currentTriple . " FILTER( xsd:datetime(" . $objValue . "_fnum_" . $filterId . ") " 
											." ". $fComp ." "    
											. "xsd:datetime(\"" 
											. $fVal . "\") ). ";
									if( $predOpts == true){
						...
						// filtering out literal for the objects
						if( $predOpts == true){
							$currentTriple = $currentTriple . "OPTIONAL { FILTER( !isLiteral(" . $objValue  . ") ) . } . ";
						}else{
							$currentTriple = $currentTriple . " FILTER( !isLiteral(" . $objValue  . ") ) . ";
						}
	
						// 						$currentTriple = $currentTriple . " FILTER( !isBlank(" . $objValue  . ") ) . ";
						$currentTriple = $currentTriple . " OPTIONAL { " . $objValue . " <http://xmlns.com/foaf/0.1/depiction> " . $objValue  . "_img  } . ";
						$currentTriple = $currentTriple . " OPTIONAL { " . $objValue . " <http://www.w3.org/2000/01/rdf-schema#label> " . $objValue  . "_label  } . ";
					}
					// 					hoochLog("Will be added to the query -> " . $currentTriple );
					$whereClause = $whereClause . $currentTriple;
						
				}
	
				if($subjConst == false){
						
					$whereClause = $whereClause . " OPTIONAL { " . $subjValue . " <http://xmlns.com/foaf/0.1/depiction> " . $subjValue  . "_img  } . ";
					$whereClause = $whereClause . " OPTIONAL { " . $subjValue . " <http://www.w3.org/2000/01/rdf-schema#label> " . $subjValue  . "_label  } . ";
						
					if($useFilters == true){
						$subjFilters = $queryElem['filters'];	
						foreach($subjFilters as $filterId => $currentFilt){
							hoochLog("Found a filter");
							$fOp = $currentFilt['op'];
							$fVal = $currentFilt['value'];
							$fComp = $currentFilt['comparison'];
							...
							if($fOp == 'regex'){
								...
								$whereClause = $whereClause . $subjValue  . $fPrd . $subjValue . "_fnum_" . $filterId . " . ";
								if( $predOpts == true){
									$whereClause = $whereClause . " } . OPTIONAL { ";
								}
								$whereClause = $whereClause . " FILTER( regex(" . $subjValue . "_fnum_" . $filterId . ", \"". $fVal . "\", \"i\" ) ) . ";
								if( $predOpts == true){
									$whereClause = $whereClause . " } . ";
								}
							}elseif ($fOp == 'math'){
								...
								$whereClause = $whereClause . " FILTER( " . $subjValue . "_fnum_" . $filterId ." ". $fComp ." ". $fVal . " ) . ";
								if( $predOpts == true){
									$whereClause = $whereClause . " } . ";
								}
							}elseif($fOp == 'datetime'){
									...
									// apply the filter
									$whereClause = $whereClause . " FILTER( xsd:datetime(" . $subjValue . "_fnum_" . $filterId . ") "
											." ". $fComp ." "
											. "xsd:datetime(\""
											. $fVal . "\") ). ";
									if( $predOpts == true){
			...
			// predicates will not be parsed, just all the nodes
		}
	}
	return $whereClause;
}

function hoochGetClassList($hoochSparqlEndpoint){
	...
	if( $db->alive() ){
		...
		if($result){
			...
			$toSend = array();
			foreach ($resultSet as $resIndex => $currentRow){
				$currentResLabel = $currentRow['class']['value'];
				$res_name=truncate_uri($currentResLabel);
				//Changes the name with the prefix if 'change_with_prefix' is true.
				if($change_with_prefix){
					$first_part_uri=truncate_uri($currentResLabel,true);
					if($label_for_uri[$first_part_uri]!=null)
						$res_name=$label_for_uri[$first_part_uri]. truncate_uri($currentResLabel);
				}
				$x = new stdClass();
				$x->label = $currentResLabel;
				$x->shortenLabel = $res_name;
				array_push($toSend,$x);
			}
			$ret->res = $toSend;
		}else{
			$ret->res = null;
			$ret->errMsg = "No class found";
		}		
	}else{
		mysparql_error($hoochSparqlEndpoint." NOT ALIVE\n".$db->error."\n-----------");
		$ret->res = null;
...
function hoochGetProperties4Class($hoochSparqlEndpoint,$c2s){

	require_once( "sparqllib.php" );
	require_once( "config.php" );

	$q = "SELECT DISTINCT  ?property WHERE { ?s a <" . $c2s  . "> ; ?property ?o . filter(!isLiteral(?o)). }";
	...
	if( $db->alive() ){
		...
		if($result){
			...
			$toSend = array();
			foreach ($resultSet as $resIndex => $currentRow){
				$currentResLabel = $currentRow['property']['value'];
				$res_name=truncate_uri($currentResLabel);
				//Changes the name with the prefix if 'change_with_prefix' is true.
				if($change_with_prefix){
					$first_part_uri=truncate_uri($currentResLabel,true);
			...
			$ret->res = null;
			$ret->errMsg = "No properties found for class " . $c2s;
		}
	}else{
		mysparql_error($hoochSparqlEndpoint." NOT ALIVE\n".$db->error."\n-----------");
		$ret->res = null;
...
function parseResultoForLog($result,$jsonIN){
	...
	$resultSet = $result->rows;
	$links2send = array();
	hoochLog("Iterating over RS for conversion. Image is " . $node_default_image);
	
	// for every Query Requested Element (QRE) build the resulting triple in wich it si included
	foreach ($jsonIN as $key => $queryElem) { // This will search in the 2 jsons
		// iterate over every query requested element
		
		// extract from the query information about the current element
		$rType = $queryElem['type'];
		if( $rType != 'resource'){
			// every requested predicate is discarded from this iteration
			// it is already included in it's own parent
			continue;
		}
		
		$rValue = $queryElem['value'];
		$rValue = ltrim ($rValue, '?');
		$rIsConst = $queryElem['isConst'];
		
		$currentResValue = $rValue;
		
		foreach ($resultSet as $resIndex => $currentRow){
			// for eache row in the result set
			// search the outer query requested elementb (QRE) in the result set
			$res_name = '';
			$currentResImage = '';
			
			// if the current QRE is not constant then 
			if($rIsConst == false){
				// get the id, image and label value form the current result set row
				if(isset($currentRow[$rValue])){
					$currentResValue = $currentRow[$rValue]['value'];
					if(isset($currentRow[$rValue. "_img"])){
						$currentResImage = $currentRow[$rValue . "_img"]['value'];
					}
					if(isset($currentRow[$rValue. "_label"])){
						$currentResLabel = $currentRow[$rValue . "_label"]['value'];
						$res_name = $currentResLabel;
					}
			...
			if($res_name == ""){
				$res_name=truncate_uri($currentResValue);
				//Changes the name with the prefix if 'change_with_prefix' is true.
				if($change_with_prefix){
					$first_part_uri=truncate_uri($currentResValue,true);
					if(isset($label_for_uri[$first_part_uri])){
						$res_name=$label_for_uri[$first_part_uri]. truncate_uri($currentResValue);
					}
				}
			}			
			
			// in any case strip the value of the current ID element by the < >
			$currentResValue = ltrim($currentResValue,'<');
			$currentResValue = rtrim($currentResValue,'>');
			
			// check if the current result ID as been created before			
			if(! isset($links2send[$currentResValue])  ){
				// if not create it now
				$links2send[$currentResValue] = new stdClass();
				$links2send[$currentResValue]->id= $currentResValue;
				$links2send[$currentResValue]->type= $rType;
				if(is_null($currentResImage) || empty($currentResImage)){
					$links2send[$currentResValue]->image= $node_default_image;
				}else{
					$links2send[$currentResValue]->image= $currentResImage;
				}
				$links2send[$currentResValue]->label= $res_name;
				$links2send[$currentResValue]->target= array();
				$links2send[$currentResValue]->target_of= array();
			}

			// check if the QRE is a resource
			
			// if it is a resource the extract the list of request predicate for the QRE
			$predicates = $queryElem['predicates'];
			foreach($predicates as $pid => $currentPred){
				$queryPredValue = $currentPred['value'];
				$queryPredValue = ltrim ($queryPredValue, '?');
					
				$predInbound = $currentPred['inbound'];
					
				if($currentPred['isConst'] != true){
					$predValue = $currentRow[$queryPredValue]['value'];
				}else{
					$predValue = $queryPredValue;
				}
				
				$prop_name=truncate_uri($predValue);
				//Changes the name with the prefix if 'change_with_prefix' is true.
				if($change_with_prefix){
					$first_part_uri=truncate_uri($predValue,true);
					if(isset($label_for_uri[$first_part_uri])){
						$prop_name=$label_for_uri[$first_part_uri]. truncate_uri($predValue);
					}
				}
				
				// strip constant tag form the predicate
				$predValue = ltrim($predValue,'<');
				$predValue = rtrim($predValue,'>');
				
				
				$predId = "";
				$parPart = $currentPred['parentId'];
				
				if($jsonIN[$parPart]['isConst'] == true){
					$dicrPart = $parPart;
				}else{
					$parPart = ltrim ($parPart, '?');
					$dicrPart = $currentRow[$parPart]['value'];
				}
				if($predInbound != true){
					$predId = $dicrPart . "-" . $predValue;
				}else{
					$predId = $predValue . "-" . $dicrPart;
				}
				
				if($dicrPart == ''){
					hoochLog("Find empty part for " . $predValue);
					hoochLog("Par part is " . $parPart);
				}
				
					
				// aggiunge il predicato alla risorsa corrente
				if(! in_array($predId,$links2send[$currentResValue]->target)  ){
					array_push($links2send[$currentResValue]->target, $predId);
				}
					
				// crea il predicato nel risultato da inviare
				if(! isset($links2send[$predId])  ){
					$x = new stdClass();
					$x->id = $predId;
					$x->label = $predValue;
					$x->shortenLabel = $prop_name;
					$x->type = 'predicate';
					$x->inbound = $predInbound;
					$x->pid = $dicrPart;
					$x->target = array();
					$links2send[$predId] = $x;
				}
...
// 					$objValue = rtrim($objValue,'>');
					$obj_name = '';
					$objImg = '';
					if($jsonIN[$objId]['isConst'] == false){
						if(isset($currentRow[$objValue])){
							$objValue = $currentRow[$objValue]['value'];
						}
						if(isset($currentRow[$objValue . "_img"])){
							$objImg = $currentRow[$objValue . "_img"]['value'];
						}
						if(isset($currentRow[$objValue . "_label"])){
							$objLabel = $currentRow[$objValue . "_label"]['value'];
							$obj_name = $objLabel;
						}
					...
					if($obj_name == ""){
						$obj_name=truncate_uri($objValue);
						//Changes the name with the prefix if 'change_with_prefix' is true.
						if($change_with_prefix){
							$first_part_uri=truncate_uri($objValue,true);
							if(isset($label_for_uri[$first_part_uri]))
								$obj_name=$label_for_uri[$first_part_uri]. truncate_uri($objValue);
						}
					...
					// se ancora non esite l'oggetto nel result set da inviare lo creo
					if(! isset($links2send[$objValue])  ){
						$links2send[$objValue] = new stdClass();
						$links2send[$objValue]->id= $objValue;
						$links2send[$objValue]->type= "resource";
						if(is_null($objImg) || empty($objImg)){
							$objImg = $node_default_image;
						}
						$links2send[$objValue]->image= $objImg;
						$links2send[$objValue]->label= $obj_name;
						$links2send[$objValue]->target= array();
						$links2send[$objValue]->target_of= array();
					}
					
					if(! in_array($predId,$links2send[$objValue]->target_of)  ){
						array_push($links2send[$objValue]->target_of, $predId);
					}
					
					if(! in_array($objValue,$links2send[$predId]->target)  ){
						array_push($links2send[$predId]->target, $objValue);
					}
				}
			}
		}
	}
	hoochLog("Conversion RS for LOG ended");
	return $links2send;
}


function hoochLog($logMsg){
	error_log("----------------------\n", 3, "/var/log/apache2/hooch.log");
	error_log($logMsg."\n", 3, "/var/log/apache2/hooch.log");
	error_log("----------------------\n", 3, "/var/log/apache2/hooch.log");
}
...
function query_count($uri,$sparql){//Counts the number of results for the uri specified.
	require_once("config.php");
	require_once( "sparqllib.php" );
	...
	foreach($sparql_endpoints as $ep){
		if($ep['endpoint']!=$sparql){			
			$db = sparql_connect( $ep['endpoint'] );
			...
			if( $db->alive() ) //else not alived.
			{
				...
				if($result){
					while( $row = sparql_fetch_array( $result ) )
					{
						// $queries_made[$j]['uri']=$uri;
						$queries_made[$j]['endpoint']=$ep['endpoint'];
						$queries_made[$j]['endpoint_name']=$ep['name'];
		...
		if($result){
			...
			while( $row = sparql_fetch_array( $result ) )
			{
				if($previous_property!=$row['property'] || $previous_uri!=$row['subject']){
					...
					$queries_made[$j]['property_name']=truncate_uri($row['property']);			
...
function search_classes($sparql){
	...
	if( $db->alive() ) //else not alived.
	{
		...
		$query="Select ?class where{?class a owl:Class .filter(!isLiteral(?class) && !isBlank(?class)) }LIMIT ".$num_classes ." ";
		$result = sparql_query( $query );
		if($result){
			$classes=array();
			while( $row = sparql_fetch_array( $result ) )
