...
elseif($_GET['query_search']!=''){//Makes the query for each sparql endpoint in config and return the list of endpoint and the number of result.
	require_once( "includes/mysparql_function.php" );
	$result=query_count($_GET['query_search'],$_GET['sparql']);
	echo JSON_ENCODE($result);
}
...
elseif($_GET['retrieve_configuration']!='') {//retrieve the configuration specified.
	...
	if($r)//Check if it's a read code.
	{
		...
		if($r){
			...
			// href="index.php?uri=http://www.disit.org/km4city/resource/048017&sparql=http://192.168.0.206:8890/sparql&keyword=Florence&multiple_search=false"
...
// }elseif ($_GET['hoochSparql']!=''){ // HOOCH added for sparql

}elseif ($_POST['hoochSparql']!=''){
	
	error_log("SPARQL query received"."\n", 3, "/var/log/apache2/hooch.log");
	// execute an arbitraty query on current EP
	// for security sake it would be better to pass the index of the endpoint instead of the endpoint url.
	// By the way all the software already buil is unsafe so...
	require_once( "includes/mysparql_function.php" );
	
	error_log("Reading Params"."\n", 3, "/var/log/apache2/hooch.log");
	
	$hoochSparqlEndpoint=$_POST['hoochSparqlEndpoint'];
	
	error_log("Endpoint  is ".$hoochSparqlEndpoint . "\n", 3, "/var/log/apache2/hooch.log");
	// 
	
	$curQuery = $_POST['query'];
	
	error_log("Cur Query  is ".$curQuery . "\n", 3, "/var/log/apache2/hooch.log");
	
// 	error_log("JSON is ". $value . "\n", 3, "/var/log/apache2/hooch.log");
	
	
	$curJSON = $_POST['jsonQ'];
	
// 	$items = JSON_DECODE($value,true,10);
	
	
// 	error_log("Iterating in ". count($items) . "\n", 3, "/var/log/apache2/hooch.log");
	
// 	for($i = 0; $i<count($items); $i++ ){
		
// 		error_log("JSON item is ".$items[i] . "\n", 3, "/var/log/apache2/hooch.log");
		
// 	}
	
// 	error_log("End Iteration" . "\n", 3, "/var/log/apache2/hooch.log");
	
	//$curQuery=$_GET['hoochQuery'];
	$result=hoochGetAllPredicates($hoochSparqlEndpoint,$curQuery, $curJSON);
	
	
	
// 	get_first_OD($_GET['first_uri'])
	
	echo JSON_ENCODE($result);
}elseif ($_POST['hoochSparqlNew']!=''){
	require_once( "includes/mysparql_function.php" );
	error_log("Test JSON"."\n", 3, "/var/log/apache2/hooch.log");
	
	$curJSON = $_POST['jsonQ'];
	//$queryId = $_POST['queryId'];
	//$resultId = "result_" . time();
	
	$clientSession = $_POST['clientSession'];
	
	$hoochSparqlEndpoint = $_POST['hoochSparqlEndpoint'];
	$result = hoochQueryJSON($hoochSparqlEndpoint,$curJSON,$clientSession);
...
// 	echo JSON_ENCODE($result);
}else if($_POST['hoochGetLiterals']!=''){
	require_once( "includes/mysparql_function.php" );
	error_log("Get Literals"."\n", 3, "/var/log/apache2/hooch.log");
	$hoochSparqlEndpoint = $_POST['hoochSparqlEndpoint'];
	$resName =  $_POST['resName'];
	$whereClause =  $_POST['whereClause'];
	$result =  hoochEnumerateLiterals($hoochSparqlEndpoint,$whereClause,$resName);
	echo JSON_ENCODE($result);
}else if($_POST['loadSparqlPanel']=='true'){
	require_once( "includes/mysparql_function.php" );
	
	$ep = $_POST['loadSparqlPanelEP'];
	$session = $_POST['loadSparqlPanelSession'];
	echo hoochLoadQueryPanel($ep,$session);
	
}else if($_POST['hoochHelperGetClassList']!=''){
	require_once( "includes/mysparql_function.php" );
	$hoochSparqlEndpoint = $_POST['hoochSparqlEndpoint'];
	echo hoochGetClassList($hoochSparqlEndpoint);

}else if($_POST['hoochHelperGetPropertiesForClass']!=''){
	require_once( "includes/mysparql_function.php" );
	$hoochSparqlEndpoint = $_POST['hoochSparqlEndpoint'];
	$targetClass = $_POST['targetClass'];
	echo hoochGetProperties4Class($hoochSparqlEndpoint,$targetClass);
}else if($_POST['hoochGetSessionId']!=''){
	require_once( "includes/mysparql_function.php" );
	echo hoochGetSessionId();
